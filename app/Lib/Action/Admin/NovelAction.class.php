...
class NovelAction extends BaseAction {
	...
	public function add()
	{
		if( $this->isPost() )
		{
	  		...
	  		{
	    		$Mnovel->add();
	    		$listURL = cookie("LURL") ? cookie("LURL") : U('/Admin/Novel/index');
	...
	public function chapter_add()
	{
		...
		if( $this->isPost() )
		{
	   		$listURL = cookie("LURL") ? cookie("LURL") : U('/Admin/Novel/view',array('id'=>$Mchapter->nid ));
	   		$res = $Mchapter->create();
	   		if( !$res )
	   		{
	    		$this->assign("jumpUrl",$listURL);
	...
	public function chapter_edit()
	{
		...
		if( $this->isPost())
		{
	  		...
	  		$res = $Mchapter->create();
	  		$listURL = cookie("LURL") ? cookie("LURL") : U('/Admin/Novel/view',array('id'=>$nid ));
	   		$this->assign("jumpUrl", $listURL );
	  		if( !$res )
	  		{
	   			$this->error( $Mchapter->getError() );
