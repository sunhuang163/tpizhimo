...
function ff_pinyin($str,$ishead=0,$isclose=1){
	...
	if(count($pinyins)==0){
		$fp = fopen(APP_PATH.'pinyin.dat','r');
		while(!feof($fp)){
			$line = trim(fgets($fp));
...
{
	return url( $action , $params , $sufix , $domain );
}
...
function ff_mysql_novel($tag)
{
	...
	if (isset( $tag['lib'] )&&$tag['ids']) {
		$where[config('database.prefix').'novel.nid'] = array('in',$tag['ids']);
	}
	if (isset( $tag['ncid']) && $tag['ncid']) {
	   $where['ncid'] = array('in', $tag['ncid']);
	}
	if(isset( $tag['lz']) && $tag['lz'] == 1){
		$where['nstate'] = array('eq', 1 );
	}
	elseif( isset( $tag['lz']) && $tag['lz'] == 2)
	{
		$where['nstate'] = ['eq' , 0];
	}
	if (isset( $tag['utime']) && $tag['utime']) {
		$year = explode(',',$tag['utime']);
		if (count($year) > 1) {
			$where['utime'] = ['between', strtotime($year[0]).','.strtotime($year[1])];
		}else{
			$where['utime'] = ['eq',$tag['year']];
		}
	}
	if ( isset( $tag['ctime']) && $tag['ctime']) {
		$year = explode(',',$tag['utime']);
		if (count($year) > 1) {
			$where['ctime'] = ['between',strtotime($year[0]).','.strtotime($year[1])];
		}else{
			$where['ctime'] = ['eq',$tag['year']];
		}
	}
	if( isset( $tag['zimu']) && $tag['zimu'])
	{
	  $where['zimu'] = ['eq' , strtoupper( $tag['zimu'])];
	}

	if( isset( $tag['title']) && $tag['title']) {
		$where['ih_novel.title'] = array('like','%'.$tag['title'].'%');
	}
	if ( isset( $tag['title']) && $tag['author']) {
		$where['ih_novel.author'] = array('like','%'.$tag['author'].'%');
	}

    $rs = \think\Db::name("Novel");

 	if( trim( $field) && !preg_match('/distinct|\*/is' , $field)  )
	{
		$sfield =  explode("," , $field );
		$sf = array(config('database.prefix').'novel.url',config('database.prefix').'novel.newurl',config('database.prefix').'novel.nid' , config('database.prefix').'novel.ncid',config('database.prefix').'novel.pic');
	    foreach( $sfield as $v )
	    {
	    	$v = trim( $v );
    ...
    else
    {
    	if( !trim($field) )
      	...
      	$list = $rs->field( $field.config('database.prefix').'ndata.*')
      			   ->join('ndata',config('database.prefix').'ndata.nid='.config('database.prefix').'novel.nid','LEFT')
                   ->where( $where )
                   ->order( $strOrder )
                   ->page( $page.",".$limit )
                   ->select();
...
function ff_upload( $fkey = 'upfile')
{
	...
	$updir = config('U_UPLOAD_DIR').date( config('U_UPLOAD_DIRPATH'));
	$reldir = __ROOT__;
	...
	if( isset( $_FILES[$fkey]) && is_uploaded_file($_FILES[$fkey]['tmp_name']))
	{
		...
		else
		{
		 	...
		 	else
			{
				...
				if( 'img' == $datt['atype'])
				{
				  	import('org.Image','extend');
				  	$image = new \org\Image;
				  	mkdirss($reldir.$updir.'thumb/',755);
				 	$imgs = NULL;
				 	$imgs = config('IMG_SIZES');
				 	$image::init('Imagick', $reldir.$updir.$fname  );
				 	$_size = $image::size();
				 	$size['w'] = $_size[0];
				 	$size['h'] = $_size[1];
				 	if( is_array($imgs) && count( $imgs ) )
				 	{
				   		...
				   		{
				   			$image::init('Imagick', $reldir.$updir.$fname  )->thumb($_v['w'] ,$_v['h'], THINKIMAGE_THUMB_SCALING );
				   			$image::save( $reldir.$updir.'thumb/s'.$_k.'_'.$fname );
				   		}
				  	}
					if(  config('IMG_WATER') ){
				   		$image::init('Imagick', $reldir.$updir.$fname  )->water(config('IMG_WATER_PIC'));
				   		$image::save( $reldir.$updir.$fname );
				 	}
				}
				$MAtt->data( $datt )->save();
				$Finfo['rcode'] = 1;
				...
				$Finfo['view_path'] = config('SITE_URL').$updir.$fname;
				$Finfo['msg'] = '文件上传成功';
			}
		}
	}
...
function authcode($string, $operation = 'DECODE', $key = '', $expiry = 0) {
	$ckey_length = 4;
	$key = md5($key != '' ? $key :  config('U_HASH_KEY') );
...
{
	$MLog = new \app\common\model\Syslog;
	$Ldata = array();
